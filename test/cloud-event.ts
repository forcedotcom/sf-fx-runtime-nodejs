...
import { parseCloudEvent, SalesforceFunctionsCloudEvent} from "../src/cloud-event";
...
describe("parseCloudEvent", () => {
  ...
  it("returns the expected objects", () => {
    ...
    const sfContext = JSON.parse(Buffer.from(cloudEvent.sfcontext.toString(), "base64").toString("utf-8"));
    const sfFunctionContext = JSON.parse(Buffer.from(cloudEvent.sffncontext.toString(), "base64").toString("utf-8"))
